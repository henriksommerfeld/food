import { QuantityUnit, type Ingredient, type Quantity } from '$lib/types'
...
export function getQuantity(
  ...
  if (ingredient.unit === QuantityUnit.pieces) return quantityForPieces(ingredient, changeFactor)

  if (isWeight(ingredient.unit)) {
    return quantityForWeight(ingredient, changeFactor)
  }
  if (isVolume(ingredient.unit)) return quantityForVolume(ingredient, changeFactor)
...
export function formattedQuantity(ingredient: Ingredient): string | null {
  ...
  if (ingredient.quantity === 0 || !ingredient.quantity) return `${ingredient.name}`
  ...
  if (ingredient.unit === QuantityUnit.pieces) return `${toFraction(ingredient)} ${ingredient.name}`
...
function volumeInMilliliters(ingredient: Quantity): number {
  ...
  if (ingredient.unit === QuantityUnit.centiliter) return ingredient.quantity * 10
  if (ingredient.unit === QuantityUnit.deciliter) return ingredient.quantity * 100
  ...
  if (ingredient.unit === QuantityUnit.tablespoon) return ingredient.quantity * 15
  throw new Error(`Unknown unit: ${ingredient.unit}`)
}
